...
export default function Navbar() {
  ...
  const navItems = ['Home', 'About Us', 'Collection', 'Customization', 'Materials', 'Contact Us'];
    ...
    >
      <div className="w-full flex items-center justify-between py-2 md:pt-5 md:pb-3 px-0">
        ...
        <motion.div 
          ...
          whileTap={{ scale: 0.95 }}
        >
          <button className="bg-gradient-to-r from-primary via-secondary to-secondary rounded-br-full rounded-tr-full px-4 sm:px-6 py-2 shadow-2xl transition-all duration-200 flex items-center">
            <img src="/images/logo.png" alt="Logo" className="w-8 h-8 sm:w-10 sm:h-10"/>
          </button>
        </motion.div>
        ...
        >
          <button className="bg-gradient-to-r from-primary via-primary to-secondary rounded-bl-full rounded-tl-full px-4 py-3 xl:py-4 shadow-2xl transition-all duration-200 flex items-center gap-2.5 text-white border border-[#4a1010]">
            ...
            <span className="text-sm xl:text-base font-medium hidden sm:inline">My Cart</span>
          </button>
        </motion.div>
      </div>
        ...
        <div className="px-4 py-6 space-y-4">
          <ul className="space-y-4">
            {navItems.map((item, index) => (
              <motion.li
                key={item}
                className="text-white text-base font-medium hover:text-gray-300 cursor-pointer transition-colors duration-200 py-2 border-b border-white/10"
